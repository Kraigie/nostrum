defmodule Mixcord.Shard.Dispatch do
  ...
  def handle_event({:GUILD_DELETE, p}, _state), do: GuildServer.delete(p.id)

  def handle_event({:GUILD_EMOJIS_UPDATE, p}, _state), do: GuildServer.emoji_update(p.guild_id, p.emojis)
  ...
  def handle_event({:GUILD_MEMBERS_CHUNK, p}, _state) do
    p.members
      |> Enum.each(fn member -> GuildServer.member_add(p.guild_id, member) end)
    p
  end

  def handle_event({:GUILD_MEMBER_REMOVE, p}, _state), do: GuildServer.member_remove(p.guild_id, p.user)

  # What is this even??
  def handle_event({:GUILD_MEMBER_UPDATE, p}, _state), do: GuildServer.member_update(p.guild_id, p.user, p.nick, p.roles)

  def handle_event({:GUILD_ROLE_CREATE, p}, _state), do: GuildServer.role_create(p.guild_id, p.role)

  def handle_event({:GUILD_ROLE_DELETE, p}, _state), do: GuildServer.role_delete(p.guild_id, p.role_id)

  def handle_event({:GUILD_ROLE_UPDATE, p}, _state), do: GuildServer.role_update(p.guild_id, p.role)
  ...
  def handle_event({:USER_UPDATE, p}, _state), do: UserCache.update(p)
