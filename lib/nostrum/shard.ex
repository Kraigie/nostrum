defmodule Nostrum.Shard do
  ...
  def start_link(gateway, token, shard_num) do
    GenServer.start_link(__MODULE__, [gateway: gateway, shard_num: shard_num, token: token])
  end

  def init(gateway: g, shard_num: s, token: t) do
    {:ok, producer_pid} = Producer.start_link(s)
