...
defmodule ExampleConsumer do
  ...
  def handle_event({:MESSAGE_CREATE, {msg}, ws_state}, state) do
    ...
    if elem(args, 0) === "!userinfo" do # Check if first arg is the command
      ...
      with true <- tuple_size(args) == 2,
            ...
            {:ok, guild} <- Nostrum.Cache.Guild.GuildServer.get(id: channel.guild_id),
                            Api.get_guild(channel.guild_id)

      do
        Api.create_message!(msg.channel_id, "#{user_id} belongs to a person named: #{user.username}\nMessage sent in: #{channel.name}\nChannel is in: #{guild.name}")
      else
        ...
        :error -> # They typed an invalid input, probably due to using letters rather than numbers.
          Api.create_message!(msg.channel_id, "Make sure the User ID is only numbers")
        false -> # There wasn't 2 elements in there, so it returned false.
